...
print(df.dtypes)
'''
scatter = plt.scatter(df.V1,df.V2, c=df.Class)
scatter.lengend()
plt.show()
...
# Maximum amount is 25,691 and the minimum transaction amount is $0, and the average amount is $88
...
# The drilled-down distribution shows a bi-modal distribution shows that there are fewer transacations at night
#There is a more distinct bi-modality for non-fraudulent transacations which indicates cycles to regular transacations that fraudulent transactions may not follow
...
# Amount looks to have a high positive skew, with a few extreme values, as indicated by the percentile distribution
